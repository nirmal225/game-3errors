...
class Game extends Component {
  ...
  renderResultsView = () => {
    ...
    return (
      <div className="results-container">
        <Results score={score} reset={this.reset} />
      </div>
    )
  }
  ...
  changeActiveImage = imageId => {
    ...
    const randomImage = imagesList[Math.floor(Math.random() * 30)]
  ...
  renderImageView = () => {
    const {activeImageId, timer} = this.state
    const image = imagesList.find(eachImage => eachImage.id === activeImageId)
    ...
    return (
      <div className="image-container">
        <img alt="match" className="active-image" src={imageUrl} />
      </div>
    )
  }

  render() {
    const {score, timer, isGameInProgress} = this.state

    return (
      <div className="app-container">
        <NavBar score={score} timer={timer} />
        {isGameInProgress ? (
          <div className="active-thumbnail-image-container">
